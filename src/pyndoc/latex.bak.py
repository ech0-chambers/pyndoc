...
class ValuedLiteral(Literal):
    ...
    def value(self, format: str = None) -> Expression:
        if format is None:
            return self._value
        self._value.format = format
        return self._value
...
def literal(expr: Any, format: str = None) -> Expression:
    ...
    if isinstance(expr, list):
        ...
        return Sequence([literal(ei) for ei in expr])
    ...
    if isinstance(expr, set):
        ...
        return Sequence([literal(ei) for ei in list(expr)])
...
def split(lines: List[str | Expression | Tuple[str | Expression]], environment = "split") -> str:
...
def function(name: str | Expression, *args: str | Expression) -> str:
    if len(args) == 1:
        return literal(name) * (literal(args[0]), )
    return literal(name) * Bracket(literal(args), scale = True)


def fmt(expr: str | Expression, format: str) -> str:
    return literal(expr, format)
